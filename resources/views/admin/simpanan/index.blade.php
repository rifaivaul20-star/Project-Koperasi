...
<div class="p-4 sm:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Daftar Simpanan</h1>
        
        <a href="{{ route('admin.simpanan.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 hover:shadow-lg hover:-translate-y-0.5 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                        ...
                        <th class="px-6 py-4 font-semibold">Jenis Simpanan</th>
                    ...
                    @forelse($simpanan as $item)
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 flex justify-center gap-4">
                                <a href="{{ route('admin.simpanan.edit', $item->id) }}" class="font-medium text-indigo-600 hover:text-indigo-900 hover:underline transition">Edit</a>
                                
                                <form action="{{ route('admin.simpanan.destroy', $item->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="font-medium text-rose-600 hover:text-rose-900 hover:underline transition" onclick="return confirm('Yakin ingin menghapus data simpanan ini?')">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-slate-500 italic bg-slate-50/30">
                                Belum ada data simpanan yang tersedia.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
